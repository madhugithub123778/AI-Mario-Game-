...
function start(){
Gamestatus = "playing";
document.getElementById("status_game").innerHTML = "Game in Progress!";
}
...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press the Start Playing button to Play", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if(Gamestatus == "playing" && noseX != "" && gameConfig.status==="start") {
    world_start.play();
